...
export default function MyCustomOrdersPage() {
  ...
  return (
    <div className="container max-w-3xl py-8">
      <h1 className="mb-6 font-heading text-2xl font-bold text-heading">Đơn hàng Custom</h1>

      <div className="space-y-4">
        {orders.map(order => (
          <Link key={order.id} to={`/account/custom-orders/${order.id}`}>
            <Card className="transition-all hover:shadow-md">
              <CardContent className="flex items-center justify-between p-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-body">{order.flowerType} · {order.personalizationContent.slice(0, 50)}...</p>
                  <p className="mt-0.5 text-xs text-caption">
                    {new Date(order.orderedAt).toLocaleDateString('vi-VN')} · Tổng: {order.totalAmount.toLocaleString('vi-VN')}₫
                  </p>
                </div>
